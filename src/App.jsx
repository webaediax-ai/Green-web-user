...
import { loadCartFromDB } from "./redux/cartSlice"; // Import the cart loading action
...
function App() {
  ...
  // Handle authentication state changes and load cart
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          await dispatch(loadCartFromDB(uid));
          console.log("Cart loaded successfully for user:", uid);
        } catch (error) {
          console.error("Error loading cart from Firestore:", error);
        }
      } else {
        dispatch(clearUser());
        // Optionally clear cart when user logs out
        // dispatch(clearCart());
      }
