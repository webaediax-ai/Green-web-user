...
function UnifiedCheckout() {
  ...
  const processOrder = async () => {
    ...
    try {
      ...
      // Get updated cart details after removing purchased items
      ...
      // Record coupon usage
      if (appliedCoupon && appliedCoupon.couponId && user) {
        try {
          await CouponService.recordCouponUsage(appliedCoupon.couponId, user.uid);
        } catch (couponError) {
    ...
    try {
      ...
      if (result.valid) {
        dispatch(applyCoupon({
          ...
          isProductSpecific: result.isProductSpecific || false,
      ...
      setCouponError(error.message || "An error occurred while validating coupon");
    } finally {
