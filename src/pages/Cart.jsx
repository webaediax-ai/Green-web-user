...
import { removeFromCart, updateQuantity, addToCart, setCart } from '../redux/cartSlice';
import { Link } from 'react-router-dom';
import { m } from 'framer-motion';
import { ShoppingBag, Trash2, Plus, Minus, ArrowRight, Loader2, Package, AlertCircle } from 'lucide-react';
...
function Cart() {
  ...
  // Load user's cart from Firestore when user logs in
  useEffect(() => {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            if (cartItems.length > 0) {
              ...
              const mergedCart = mergeCarts(cartItems, userCart);
  ...
  const mergeCarts = (localCart, dbCart) => {
    ...
    localCart.forEach(localItem => {
      ...
      if (existingIndex >= 0) {
        // Sum quantities if item exists in both
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(updateCartInDB, 500);
    return () => clearTimeout(timeoutId);
  }, [cartItems, user, syncing]);

  const handleRemove = async (productId) => {
    if (!user) {
      toast.info("Please sign in to save your cart");
    }
  ...
  const cartDetails = cartItems
    ...
    .filter(Boolean)
    .filter(item => item.product.stock > 0); // Filter out out-of-stock items

  // Check for out-of-stock items
  const outOfStockItems = cartItems
    .map(item => {
      const product = products.find(p => p.id === item.productId);
      return product && product.stock === 0 ? { ...item, product } : null;
    })
    .filter(Boolean);
                ...
                <ul className="text-sm text-red-700 space-y-1">
                  {outOfStockItems.map(item => (
                    <li key={item.productId}>
                      • {item.product.name} - Please remove to proceed
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {cartItems.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              {!user ? (
                <>Looks like you haven't added any items to your cart yet. <Link to="/signin" className="text-gray-900 font-semibold underline">Sign in</Link> to see saved items.</>
              ) : (
                ...
                {!user && (
                  <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-xs text-yellow-800">
                      <strong>Note:</strong> Your cart won't be saved if you leave.{" "}
                      <Link to="/signin" className="font-semibold underline">
                        Sign in
                      </Link>{" "}
                      to save items permanently.
                    </p>
                  </div>
                )}

                <Link
                  to={outOfStockItems.length > 0 ? "#" : "/checkout"}
                  onClick={(e) => {
                  ...
                  className={`w-full bg-gray-900 text-white py-4 px-6 rounded-xl font-bold hover:bg-gray-800 transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl mb-3 ${
                    outOfStockItems.length > 0 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  Proceed to Checkout
                  <ArrowRight className="w-5 h-5" />
